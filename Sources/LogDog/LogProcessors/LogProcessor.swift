...
extension LogProcessor {
    ...
    public func combine<P>(_ other: P) -> MultiplexLogProcessor<Self, P> where P: LogProcessor, P.Input == Output {
        .init(self, other)
    }
}

public func +<A, B>(_ a: A, _ b: B) -> MultiplexLogProcessor<A, B> where A: LogProcessor, B: LogProcessor, A.Output == B.Input {
    a.combine(b)
}
